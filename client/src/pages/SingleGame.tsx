...
import { CircleAlert, RotateCcw, Lightbulb, Target, X, Home, Gamepad2 } from 'lucide-react';
...
export default function SingleGame() {
  ...
  const guess = async (characterId: number) => {
    ...
    try {
      const res = await api.post(`/game/${gameId}/guess`, { characterId });
    ...
    <Page
      ...
      actions={
        <>
          ...
          >
            <Lightbulb size={15} />
            <span className="btn-text">{revealing ? t('game.processing') : t('game.reveal')}</span>
          </button>
        </>
      }
      showHome={false}
