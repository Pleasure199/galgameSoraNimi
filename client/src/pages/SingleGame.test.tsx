...
describe('SingleGame UX', () => {
  ...
  it('shows reveal busy state on the action button and top status bar', async () => {
    ...
    const giveup = deferred<{
      data: {
        answer: { name: string; work: string; company: string; releaseYear: number; gender: string; cv: string; hairColor: string; hairLength: string };
      };
    }>();
    ...
    giveup.resolve({
      data: {
        answer: { name: '牧瀬紅莉栖', work: 'STEINS;GATE', company: 'MAGES.', releaseYear: 2009, gender: '女', cv: '今井麻美', hairColor: '红色', hairLength: '长发' },
      },
    });
  ...
  it('keeps an unrecorded settlement warning visible in the answer dialog', async () => {
    ...
    expect(result).toHaveTextContent('牧瀬紅莉栖');
