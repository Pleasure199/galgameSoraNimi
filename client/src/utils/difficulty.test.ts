...
describe('difficulty helpers', () => {
  it('maps known difficulties to distinct icons and colors', () => {
    ...
    expect(difficultyIcon('hard')).toBe(Skull);
    expect(difficultyIcon('complete')).toBe(Trophy);
    ...
    expect(difficultyColor('hard')).toBe('var(--danger)');
    expect(difficultyColor('complete')).toBe('var(--warning)');
  ...
  it('resolves localized labels and descriptions', () => {
    ...
    expect(difficultyDescription(t, 'beginner')).toBe('高销量作品的主要角色 · 熟悉规则');
    expect(difficultyLabel(t, 'easy')).toBe('简单版');
    expect(difficultyDescription(t, 'easy')).toBe('中高销量作品的主要角色 · 快速上手');
