...
import { getCharacterList, searchCharacterList, subscribeCharacterList } from '../api/characterList';
...
/**
 * 底部输入栏:角色名输入 + 提交按钮,自动补全列表从输入框上方弹出(原版布局)。
...
export default function GuessInputBar({
  ...
  const applyQuery = useCallback((
    ...
    const next = searchCharacterList(list, query);
    setItems(next);
  ...
  useEffect(() => {
    ...
    void getCharacterList().then(applyCharacterList).catch((error) => toast.error(errMsg(error)));
    return unsubscribe;
  }, [applyCharacterList]);
      ...
      <form className="input-bar" onSubmit={submit}>
        <input
          ...
          onFocus={() => {
            ...
            void getCharacterList().catch(() => undefined);
            onFocusChange?.(true);
          }}
          onBlur={() => {
