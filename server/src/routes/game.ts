...
import { Character, GuessFeedback } from '../types';
import { compareGuess, MAX_GUESSES } from '../services/gameService';
import { getCharacter, getEnabledCharacter, isDifficultyAvailable, pickCachedTarget } from '../services/characterCache';
  ...
  if (shouldPersist) {
    await db('games')
      .insert({
        ...
        mode: game.mode,
        guesses: JSON.stringify(game.guesses.map((guess) => guess.characterId)),
        guess_times: JSON.stringify(game.guessTimes),
        first_guess_character_id: game.guesses[0]?.characterId ?? null,
        status,
...
router.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    const response = await withKeyLock(`single-start:${owner.identityKey}:${mode}`, async () => {
      ...
      if (!target) throw new HttpError(500, 'EMPTY_CHARACTER_POOL');
