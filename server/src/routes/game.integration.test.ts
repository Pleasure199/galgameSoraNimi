...
import { isRedisUp } from '../test/redisUp';
...
describe.skipIf(!redisUp)('single-player settlement soft limit', () => {
  beforeAll(async () => {
    await initDb();
    await initRedis();
  ...
  it('persists accepted guess times relative to the game start', async () => {
    ...
    const targetCharacterId = Number(JSON.parse(activeRaw!).targetCharacterId);
    try {
