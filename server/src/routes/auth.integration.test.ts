...
describe('cookie authentication', () => {
  ...
  it('binds guest claims and revokes logout immediately', async () => {
    ...
    const guestToken = cookie.split('; ').find((item) => item.startsWith('tianyiba_guest='))!.split('=')[1];
    ...
    const guestIdValue = crypto
      ...
      .update('tianyiba-guest-id-v1\0', 'ascii')
    ...
    expect(cookie).toContain('tianyiba_session=');
    expect(cookie).toContain('tianyiba_refresh=');
    ...
    expect(issuedCookies.find((item) => item.startsWith('tianyiba_session=')))
      .toContain('Path=/');
    expect(issuedCookies.find((item) => item.startsWith('tianyiba_refresh=')))
      .toContain('Path=/api/auth');
    const authToken = cookie.split('; ').find((item) => item.startsWith('tianyiba_session='))!.split('=')[1];
    ...
    const refreshToken = cookie.split('; ').find((item) => item.startsWith('tianyiba_refresh='))!.split('=')[1];
    ...
    const legacyAccessOnlyCookie = cookie
      .split('; ')
      .filter((item) => !item.startsWith('tianyiba_refresh='))
      .join('; ');
    ...
    cookie = cookie
      .split('; ')
      .map((item) => item.startsWith('tianyiba_session=')
        ? `tianyiba_session=${expiredAccess}`
        : item)
      .join('; ');
    ...
    expect(setCookies(expiredOptional.response).join(';')).not.toContain('tianyiba_guest=');
    ...
    const refreshedAccess = cookie
      .split('; ')
      .find((item) => item.startsWith('tianyiba_session='))!
      .split('=')[1];
    ...
    expect(setCookies(result.response).find((item) => item.startsWith('tianyiba_guest=')))
    ...
    const refreshBeforeLogout = cookie
      .split('; ')
      .find((item) => item.startsWith('tianyiba_refresh='))!;
