...
let available = false;
// Whether Redis has ever connected during this process lifetime. Before the
// first successful connect we fail fast (memory fallback), after that we keep
// reconnecting so transient outages do not strand a live deployment.
...
/** Isolate game and presence state from HTTP and rate-limit traffic. */
...
function closeClient(client: Client): Promise<void> {
  // quit() performs a graceful QUIT handshake and HANGS on clients that failed
  // to connect (their socket sits in a reconnect loop). Only use it for live
  // connections; force-destroy everything else.
  if (!client.isReady) {
    client.disconnect();
